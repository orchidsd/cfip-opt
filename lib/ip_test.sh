...
ip_test_run() {
    ...
    if [[ "$speed_test_enabled" == "true" ]]; then
        ...
        if [[ -n "$speed_test_url" ]]; then
            if [[ "$speed_test_url" =~ ^https?:// ]]; then
                url="$speed_test_url"
            else
                [[ "$port" =~ ^(443|8443|2053|2083|2087|2096)$ ]] && proto="https://" || proto="http://"
                url="${proto}${speed_test_url}"
            fi
        else
            [[ "$port" =~ ^(443|8443|2053|2083|2087|2096)$ ]] && url="https://speed.cloudflare.com/__down?bytes=200000000" || url="http://speed.cloudflare.com/__down?bytes=200000000"
        fi
        cfst_args+=("-url" "$url")
    else
        cfst_args+=("-dd")
